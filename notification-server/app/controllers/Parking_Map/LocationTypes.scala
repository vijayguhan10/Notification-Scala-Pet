...
class LocationTypeController @Inject() (
  ...
  implicit val locationTypeFormat: OFormat[LocationType] =
      ...
      .map {
        ...
        case None          =>
          NotFound(
            Json.obj(
              "status" -> "Error",
              "message" -> s"Location Type with ID $id not found."
            )
          )
      }
  ...
  /** GET /api/location-types?limit=50&offset=0
    */
  def list(limit: Int, offset: Int): Action[AnyContent] = Action.async {
    implicit request =>
      service
        .listLocationTypes(limit, offset)
        .map { list =>
          Ok(Json.toJson(list))
        }
        .recover { case ex: Throwable =>
    ...
    implicit request =>
      (request.body \ "typeName").validate[String] match {
        case JsSuccess(newName, _) =>
          service
            .updateLocationTypeName(id, newName)
            .map { wasUpdated =>
              if (wasUpdated)
                Ok(
                  Json.obj(
                ...
                NotFound(
                  Json.obj(
                    "status" -> "Error",
                    "message" -> s"Location Type $id not found."
                  )
                )
            }
            .recover {
        ...
        case JsError(_) =>
          Future.successful(
            BadRequest(
              Json.obj(
                "status" -> "Error",
                "message" -> "Missing or invalid 'typeName' string field."
              )
            )
          )
      }
      ...
      .map { wasDeleted =>
        if (wasDeleted)
          Ok(
            Json.obj(
              "status" -> "Success",
              "message" -> s"Location Type $id deleted."
            )
          )
        else
          NotFound(
