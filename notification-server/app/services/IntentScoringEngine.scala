...
class IntentScoringEngine @Inject() (
  ...
  // Incremental model:
  // each signal is a 0/1 bit; when it flips 0->1 we add its weight once.
  private val signals: List[Signal] = List(
    Signal(bitOffset = 0, weight = 20, active = _.parkingSearches >= 5),
    Signal(bitOffset = 1, weight = 15, active = _.slotViews >= 20),
    Signal(bitOffset = 2, weight = 25, active = _.bookingAttempts >= 1),
    Signal(bitOffset = 3, weight = 10, active = _.avgScrollDepth >= 70),
    Signal(
      bitOffset = 4,
      weight = 15,
      active = e => Option(e.lastLocation).exists(_.trim.nonEmpty)
    ),
    Signal(bitOffset = 5, weight = 15, active = _.sessionDuration >= 600)
  )
      ...
      redis.withJedis { jedis =>
        ...
        val newScore: Long =
          if (delta != 0L) {
            ...
            if (current == null) 0L else current.toLongOption.getOrElse(0L)
          }
