...
class IntentScoringEngine @Inject() (
  ...
  /** Represents a single behavioral scoring rule.
    *
    * @param bitOffset
    *   Bitmap position used to track whether this signal has already
    *   contributed to the user's score.
    *
    * @param weight
    *   Score added when this signal becomes active for the first time.
    *
    * @param active
    *   Predicate that evaluates whether the signal is active for a given user
    *   activity event.
    */
  ...
  /** Incremental behavioral scoring model.
    ...
    * Bitmap semantics:
    *
    * 0 -> signal not rewarded yet 1 -> signal already rewarded
    *
  ...
  /** Redis bitmap key storing rewarded signal bits.
    ...
    *
    * Bitmap tracks which behavioral milestones have already contributed to the
    * score.
    */
  ...
  /** Processes a user activity event and incrementally updates the user's
    * intent score.
    *
    * Flow:
    *
    ...
    * Redis data model:
    *
    * intent:bits:<userId> -> bitmap of rewarded signals intent:score:<userId>
    * -> accumulated score
    *
  ...
  def updateAndGet(event: UserActivityEvent): IntentScoreResult = {
    ...
    /** Execute scoring logic inside a safely managed Redis connection borrowed
      * from the Jedis pool.
      *
    ...
    val score =
      redis.withJedis { jedis =>
        ...
        signals.foreach { signal =>
          /** Check whether the current behavioral rule becomes active for this
            * event.
            */
          if (signal.active(event)) {
            ...
            /** Redis SETBIT operation:
              ...
              * If previous bit:
              *
              * false -> signal never rewarded before true -> signal already
              * rewarded earlier
              */
            val previousBit =
              jedis.setbit(keyBits, signal.bitOffset.toLong, true)

            /** Only reward score if signal becomes active for the FIRST time.
              *
        ...
        /** Update accumulated score only if newly activated signals were
          * detected.
          */
        val newScore: Long =
          if (delta != 0L) {
  ...
  /** Maps numeric score into business-friendly intent classification.
    *
    * Score ranges:
    *
    * < 30 -> low < 60 -> moderate < 80 -> high >= 80 -> immediate
    */
